require ['public/assets/javascripts/lib/managers/validation', 'public/assets/javascripts/lib/utils/validation_rules'], (Validate, ValidationRules) ->
  ...
  describe 'Lp Form Validation', ->
    ...
    describe 'On Submit error handling', ->
      ...
      xit 'will remove an error from the list on correction', ->
        @elem1.val('test').blur()
        count = @form.find('li').length
        expect(count).toEqual(1)

      xit 'will add an error back into the list', ->
        @elem1.val('test').blur()
        @elem2.val('').blur()
        @form.submit()
        count = $('ul.err-container').find('li').length
